Derive整个表格数据至另一个表（不分logline或非logline）数据到CR访视相同页面
...
//Get data point form edit check
            ...
            //Get the current data page records
            ...
            //Add the same number of Loglines to the target data page

            while (rdsBL.Count > dpgCR.Records.Count) dpgCR.AddLogRecord();

 

            //Active and Inactive records in the target data page according to the source data page
...

            {
...

                {
                    ...
                    //Derive data to the target data page
...

        {
            ...
            //Loops all the fields in the source data page 
...

            {
...

                {
                    ...
                    //Derive the source data to the target data point
